...
plot_X_filename = os.path.join(output_path, "X_N%d_%0.2f_%0.2f_%0.2f_%0.2f.png"    % (N, f_a, f_b, g_a, g_b))
plot_H_filename = os.path.join(output_path, "H_N%d_%0.2f_%0.2f_%0.2f_%0.2f.png"    % (N, f_a, f_b, g_a, g_b))
data_filename   = os.path.join(output_path, "data_N%d_%0.2f_%0.2f_%0.2f_%0.2f.pkl" % (N, f_a, f_b, g_a, g_b))

def one_step(H,X):
    # X_t from H_t
    # H_{t+1} from X_t
    X = g_b + H * np.random.exponential(scale=g_a, size=H.shape)
